...
/// <summary>
/// An offscreen geometric exclusion guard. This is never a visible skin mesh:
/// the composite intersects its feature masks with the projected triangle union.
/// Projecting the actual surface preserves nose/lip concavities on a profile;
/// a convex hull or the usual face-oval outline would include background pixels.
/// Pixel coordinates use the same texture UV convention as the camera composite.
/// </summary>
public sealed class FacelessSurface : IDisposable
{
    const int LandmarkCount = 468;
    const int VertexCount = LandmarkCount + 3;
    readonly Vector3[] _vertices = new Vector3[VertexCount];
    Mesh _mesh;
    Material _material;
    RenderTexture _coverage;
    bool _valid;
    bool _reportedFailure;

    // Official Google MediaPipe canonical face topology, Apache License 2.0.
    ...
    // The 468 indices are the same Face Landmarker landmark indices.
    static readonly int[] SurfaceTriangles =
    {
    ...
    static readonly int[][] FeatureLoops =
    {
        new[] {33,7,163,144,145,153,154,155,133,173,157,158,159,160,161,246},
        new[] {263,249,390,373,374,380,381,382,362,398,384,385,386,387,388,466},
        new[] {78,95,88,178,87,14,317,402,318,324,308,415,310,311,312,13,82,81,80,191}
    };

    /// <summary>Null until a successful render, or after an invalid input frame.</summary>
    public Texture Texture { get { return _valid ? _coverage : null; } }

    public bool Render(Vector2[] pixelPoints, int width, int height, Shader shader)
    {
        _valid = false;
        if (pixelPoints == null || pixelPoints.Length < LandmarkCount ||
            width < 1 || height < 1 || width > SystemInfo.maxTextureSize ||
            height > SystemInfo.maxTextureSize || shader == null || !shader.isSupported)
            return false;

        for (int i = 0; i < LandmarkCount; i++)
        {
            Vector2 p = pixelPoints[i];
            if (float.IsNaN(p.x) || float.IsNaN(p.y) ||
                float.IsInfinity(p.x) || float.IsInfinity(p.y)) return false;
            _vertices[i] = new Vector3(p.x / width, p.y / height, 0);
        }
        for (int feature = 0; feature < FeatureLoops.Length; feature++)
        {
            Vector3 center = Vector3.zero;
            int[] loop = FeatureLoops[feature];
            for (int i = 0; i < loop.Length; i++) center += _vertices[loop[i]];
            _vertices[LandmarkCount + feature] = center / loop.Length;
        }
        ...
        try
        {
            EnsureResources(width, height, shader);
            _mesh.vertices = _vertices;
            Graphics.SetRenderTarget(_coverage);
            GL.sRGBWrite = false;
            GL.Clear(false, true, Color.clear);
            if (!_material.SetPass(0)) return false;
        ...
        catch (Exception error)
        {
            if (!_reportedFailure)
            {
                Debug.LogWarning("FacelessSurface: could not render surface guard. " + error.Message);
    ...
    void EnsureResources(int width, int height, Shader shader)
    {
        ...
        if (_mesh == null)
        {
            _mesh = new Mesh
            {
                name = "Faceless projected triangle union",
                hideFlags = HideFlags.HideAndDontSave
            };
            _mesh.MarkDynamic();
            _mesh.vertices = _vertices;
            int fanIndexCount = 0;
            for (int i = 0; i < FeatureLoops.Length; i++) fanIndexCount += FeatureLoops[i].Length * 3;
            var indices = new int[SurfaceTriangles.Length + fanIndexCount];
            Array.Copy(SurfaceTriangles, indices, SurfaceTriangles.Length);
            int cursor = SurfaceTriangles.Length;
            for (int feature = 0; feature < FeatureLoops.Length; feature++)
            {
                int[] loop = FeatureLoops[feature];
                for (int i = 0; i < loop.Length; i++)
                {
                    indices[cursor++] = LandmarkCount + feature;
                    indices[cursor++] = loop[i];
                    indices[cursor++] = loop[(i + 1) % loop.Length];
                }
            }
            _mesh.triangles = indices;
            _mesh.bounds = new Bounds(new Vector3(0.5f, 0.5f, 0), new Vector3(4, 4, 1));
        }
        if (_coverage != null && (_coverage.width != width || _coverage.height != height))
        {
            _coverage.Release();
            ReleaseObject(_coverage);
            _coverage = null;
        }
        if (_coverage == null)
        {
            _coverage = new RenderTexture(width, height, 0,
                RenderTextureFormat.ARGB32, RenderTextureReadWrite.Linear)
            {
                name = "Faceless surface exclusion mask",
                hideFlags = HideFlags.HideAndDontSave,
                useMipMap = false,
        ...
        if (!_coverage.IsCreated() && !_coverage.Create())
    ...
    {
        _valid = false;
        if (_coverage != null) _coverage.Release();
        ReleaseObject(_coverage);
        ReleaseObject(_material);
        ReleaseObject(_mesh);
        _coverage = null;
